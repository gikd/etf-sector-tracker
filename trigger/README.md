...
## 발사 시각 (`wrangler.toml`)
장중에는 **매시간** 쏘고, 개장/마감은 정밀 포인트로 한 번 더 잡는다.
| 구간 | UTC cron | 뜻 |
|---|---|---|
| 한국장 매시간 | `5 0-6 * * 1-5` | 09~15 KST 매시 05분(개장 00:05 포함) |
| 한국장 마감 | `35 6 * * 1-5` | 15:35 KST(06:35 UTC) |
| 미국장 매시간 | `5 13-21 * * 1-5` | 13~21 UTC 매시 05분(마감 20:05/21:05 포함) |
| 미국장 개장 | `35 13,14 * * 1-5` | 09:35 ET(서머 13:35 / 윈터 14:35) |

미국은 서머/윈터 두 시각 모두 발사한다. 비해당·중복 발사는 무해 — `update.yml` 은 데이터가 실제로 바뀐 경우에만 커밋한다.
